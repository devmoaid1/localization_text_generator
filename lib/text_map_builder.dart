...
class TextMapBuilder {
  ...
  void generateTextMap(List<Set<String>> texts,List<File> files) {
    ...
    for(int i=0; i<texts.length;i++){
      ...
      for (int g = 0; g < texts[i].length; g++) {
        ...
        _textsMap.last['${parent=='lib'?'':'$parent-'}${files[i].path.split('/').last.split('.').first}-$g'] = texts[i].elementAt(g);
              // TODO: add file content with replaced data.
              // fileContent=fileContent.replaceFirst(texts[i].elementAt(g),'');
