...
class TextMatcher {
  ...
  void matchAndExtractTexts(String fileContent,String path) {
    // RegExp removeCommentsExpression = RegExp(r'''\/\/[^\r\n]*|\/\*(?:(?!\*\/)[\s\S])*\*\/''');
    // final commentsMatches=removeCommentsExpression.allMatches(fileContent);
    // for(Match match in commentsMatches){
    // String? comment= match.group(0);
    // if(comment!=null){
    ...
    final textExtractorExpression = RegExp(
        r'''(?<!import\s)(?<!fontFamily:\s)(?<!{\s)(?<!//\s)(?<!Key\()(?<!print\()(?<!log\()(?<!RegExp\()(?<!^\/\/\/?\s*)(['"])((?:\\\1|(?!\1).)*)\1''',
        multiLine: true);
